...
候选与请求之间使用 CSR 风格的稀疏索引：每个实际存在的 `(缓存, 视频)` 候选只保存它能够改善的请求编号和对应缓存延迟。收益更新不再扫描缓存连接的全部终端；索引使用 `int[]`、`short[]` 等连续数组，以控制大型数据集上的对象数量和内存开销。
...
- `GreedySolver`：构建候选到请求贡献的紧凑稀疏索引，并执行动态边际收益密度贪心。
